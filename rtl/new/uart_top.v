...
module uart_top (
    ...
    wire       s = rx_done && (rx_data == 8'h73); // s

    wire [4:0] hour = watch_time[23:19];
    wire [5:0] min = watch_time[18:13];
    wire [5:0] sec = watch_time[12:7];
    wire [6:0] msec = watch_time[6:0];
...
module ascii_decoder (
    ...
    always @(posedge clk or posedge reset) begin
        if (reset) begin
            ...
            if (rx_done) begin
                case (rx_data)
                    8'h72:   in_btn_r <= 1'b1;  //r
                    8'h6E:   in_btn_n <= 1'b1;  //n
                    8'h63:   in_btn_c <= 1'b1;  //c
                    8'h74:   in_btn_sr <= 1'b1; //t
                    8'h68:   in_btn_dht <= 1'b1;  //h
                    default: ;
                endcase
            end
        end
